...
public class IndexStreamParallel<T> extends IndexStream<T> {
    ...
    public Optional<T> reduce(BinaryOperator<T> accumulator) {
        return IntStream.range(min, max).parallel().mapToObj(index -> f.apply(index).orElse(null))
                .filter(x -> x != null)
                .reduce(accumulator);
    }
    ...
    public BasicStream<T> filter(Predicate<T> predicate) {
        return new IndexStreamParallel<>(i -> f.apply(i).filter(predicate), min, max);
    }

    @Override
    public BasicStream<T> limit(long maxSize) {
        return new IndexStreamParallel<>(f, min, min + (int) maxSize);
    }

    @Override
    public <R> BasicStream<R> map(Function<T, R> mapper) {
        return new IndexStreamParallel<>(i -> f.apply(i).map(mapper), min, max);
    }
